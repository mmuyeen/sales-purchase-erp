...
import { createSupplier, getSupplier, updateSupplier } from '../../api/suppliers.js';
...
export default function SupplierForm() {
  ...
  useEffect(() => {
    if (isEdit) {
      getSupplier(id).then((res) => setForm({ ...EMPTY, ...res.data })).catch((err) => setError(err.message));
    }
  ...
  return (
    <div>
      <div className="page-header"><h1>{isEdit ? 'Edit Supplier' : 'Add Supplier'}</h1></div>

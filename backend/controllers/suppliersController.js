...
export async function list(req, res) {
  ...
  const { rows } = await getPool().query(
    `select * from suppliers ${where} order by supplier_name asc`,
    params
  );
...
export async function create(req, res) {
  ...
  const row = await withTransaction(async (client) => {
    const supplierCode = await generateNumber(client, 'SUP');
    const { rows } = await client.query(
      `insert into suppliers (supplier_code, supplier_name, address, state, city, pincode, gst_number, phone, email)
       values ($1,$2,$3,$4,$5,$6,$7,$8,$9) returning *`,
      [supplierCode, input.name, input.address, input.state, input.city, input.pincode, input.gstNumber, input.phone, input.email]
    );
    return rows[0];
  });
